...
create_job_py_path = os.path.abspath(os.path.join(BASE_DIR, "../../simulation/fedavg_create_job_runner.py"))

def main(config_path, job_id, aggr_function, num_global_iteration, num_local_epoch):
    """Config 파일을 로드하여 Nautilus 배포 및 실행"""
    # Config 파일 로드
    config_full_path = os.path.abspath(os.path.join(BASE_DIR, "../../workspace/configs", config_path))
    ...
    for i in range(number_of_client):
        ...
        train_py_path = os.path.abspath(os.path.join(BASE_DIR, "../../simulation/src", f"{project_id}.py"))
