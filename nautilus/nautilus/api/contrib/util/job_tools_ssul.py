...
from contrib.pipline import connect_contrib_evaluation
from .api_tools import http_post

class nt_FedAvg_Pack(nt_FedAvg):
    def __init__(self, num_clients, client_lists, num_rounds, persistor_id, initial_model, server_url, contribution_method, project_id):
        super().__init__(
    ...
    def nt_get_client_information(self, results):
        '''
        Meta Fields: client_name, accuracy, metric, data_size, feature_embedding, props
        '''
        client_training_result_data = []

        for _result in results:
            meta = _result.meta
            metrics = _result.metrics if hasattr(_result, "metrics") else None

            client_data = {
                "client_name": meta.get("client_name", AppConstants.CLIENT_UNKNOWN),
                "accuracy": meta.get("accuracy", None),
            }

            client_training_result_data.append(client_data)

        return client_training_result_data

    def nt_calculate_test_accuracy(self, model, model_weight, DEVICE, test_data):
        test_model = model
    ...
    def run(self) -> None:
        ...
        for self.current_round in range(self.start_round, self.start_round + self.num_rounds):
            ...
            # 1. 결과 디버깅 출력
            client_result_information = self.nt_get_client_information(results)
            ...
            # 2. 테스트 정확도 평가
            eval_model = initial_model
            model_weights = aggregate_results.params
            eval_model.load_state_dict({k: torch.tensor(v) for k, v in model_weights.items()})
            eval_model.to(DEVICE)
            test_accuracy = self.nt_calculate_test_accuracy(eval_model, model_weights, DEVICE, testloader)
            
            self.save_model(model)

            # payload 생성
            payload = {
                "event_type": "server_result_created",
                "data": {
                    ...
                    "client_results": client_result_information,
                    "contrib_results": contrib_results
                }
            }
            ...
            if response:
                ...
                self.info("Results 및 Aggregate Results 전송 실패")
            

        self.info("Finished FedAvg.")
        
        # ✅ 모든 라운드 종료 후 서버에 종료 알림 전송
        final_payload = {
            "event_type": "training_completed",
            "data": {
                "project_id": self.project_id,
                "status": "completed",
                "message": "All training rounds completed successfully",
                "total_rounds": self.num_rounds,
                "final_round": self.current_round
            }
        }

        print("\n✅ Final Training Summary (All Rounds Completed):\n" + json.dumps(final_payload, indent=2, default=str))
        final_response = http_post(self.server_url, final_payload)

        if final_response:
            self.info(f"Training 완료 알림 전송 성공: {final_response}")
        else:
            self.info("Training 완료 알림 전송 실패")
...
class nt_Job_controller(BaseFedJob):
    def __init__(
        ...
        contribution_method: str = "robust_volume",
        server_url: str = "http://172.17.0.1:8000/nautilus/v1/result",# 서버ip 주소 입력하기(pod에서 localhost는 본인으로
        project_id: str = "p-kr-federated-learning-pj"
    ):
        ...
        if FL_method == "FedAvg":
            controller = nt_FedAvg_Pack(num_clients=n_clients, 
                    client_lists = client_lists, 
                    num_rounds=num_rounds,
