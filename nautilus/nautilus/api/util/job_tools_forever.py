...
class nt_model_controller(Controller, FLComponentWrapper, ABC):
    def __init__(
        ...
        # config data
        #print('ignore_result_error : ',ignore_result_error)
...
class nt_FedAvg_Pack(nt_FedAvg):
    ...
    def run(self) -> None:
        ...
        for self.current_round in range(self.start_round, self.start_round + self.num_rounds):
            ...
            print('\n\n\n\n\n\n\n here : ',results)
            nt_contrib_individual(initial_model, results, DEVICE, testloader ,mode='s_norm')
            #nt_contrib_loo(initial_model, results, DEVICE, testloader, 'basic')
            #nt_contrib_loo(initial_model, results, DEVICE, testloader, 'weighted',[0.6,0.4])
            #nt_contrib_shap(initial_model, results, DEVICE, testloader, 'basic')
